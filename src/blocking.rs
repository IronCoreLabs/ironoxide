...
// Main impl block: methods whose signatures are already UniFFI-compatible.
// When the `uniffi` feature is enabled, this block is exported via `uniffi::export`.
// All param/return types here use &self, &CustomType, &[u8], &[T], &str, Vec<u8>,
// Option<T>, bool, Result<ObjectType>, etc.
...
impl BlockingIronOxide {
    ...
    // -- User operations (instance methods only) --
...
// Methods whose signatures are NOT UniFFI-compatible (returns ref to Object, usize,
// tuple return, Duration param, associated functions, or beta-gated).
// These are only available when building without the `uniffi` feature.
#[cfg(not(feature = "uniffi"))]
impl BlockingIronOxide {
...
// UniFFI-compatible variants of methods that have incompatible native signatures.
// These use Arc wrappers, u64 instead of usize, RotateAllResult instead of tuple, etc.
#[cfg(feature = "uniffi")]
#[uniffi::export]
impl BlockingIronOxide {
    /// Get the `DeviceContext` instance that was used to create this SDK instance
