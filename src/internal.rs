...
pub enum IronOxideErr {
    #[error("'{field_name}' failed validation with the error '{err}'")]
    ValidationError { field_name: String, err: String },

    #[error("{message}")]
    DocumentHeaderParseFailure { message: String },

    #[error("WrongSizeError(actual: {actual_size:?}, expected: {expected_size:?})")]
    WrongSizeError {
    ...
    #[error(transparent)]
    AesError(#[from] aws_lc_rs::error::Unspecified),

    #[error("Provided document is not long enough to be an encrypted document.")]
    AesEncryptedDocSizeError,

    #[error("Got an unexpected Recrypt EncryptedValue: '{msg}'")]
    InvalidRecryptEncryptedValue { msg: String },

    #[error("Recrypt operation failed with error '{msg}'")]
    RecryptError { msg: String },

    #[error("Operation failed with error '{msg}'")]
    UserDoesNotExist { msg: String },

    #[error("User or group {user_or_group} does not exist.")]
    UserOrGroupDoesNotExist {
        user_or_group: document_api::UserOrGroup,
    },

    #[error("SDK initialization failed. Underlying cause '{cause}'")]
    InitializeError { cause: String },

    #[error("Request failed with HTTP status code '{http_status:?}' message '{message}' and code '{code:?}'")]
    RequestError {
        message: String,
    ...
    /// This is used if the response from the server was an error. In that case we know that the
    /// format of the errors will be `ServerError`.
    #[error("Request failed with HTTP status code '{http_status:?}' errors list is '{errors:?}' and code '{code:?}'")]
    RequestServerErrors {
    ...
    #[error("Expected at least one TransformBlock in transformed value but received none.")]
    ...
    #[error("You are not an administrator of group '{}'", id.id())]
    NotGroupAdmin { id: GroupId },

    /// No policy exists for the segment
    #[error("No policy is defined. Please visit https://admin.ironcorelabs.com/policy to set a policy")]
    PolicyDoesNotExist,

    /// Protobuf encode/decode error
    #[error(transparent)]
    ProtobufSerdeError(#[from] ProtobufError),

    /// Protobuf decode succeeded, but the result is not valid
    #[error("Protobuf validation failed with '{msg}'")]
    ProtobufValidationError { msg: String },

    #[error("Edeks and EncryptedDocument do not match. Edeks are for DocumentId({edek_doc_id}) and SegmentId({edek_segment_id}) and Encrypted Document is DocumentId({edoc_doc_id}) and SegmentId({edoc_segment_id})")]
    ...
    #[error("User private key rotation failed with '{msg}'")]
    UserPrivateKeyRotationError { msg: String },

    #[error("Group private key rotation failed with '{msg}'")]
    ...
    #[error(transparent)]
    CacheSerdeError(#[from] postcard::Error),

    #[error("AES-GCM decryption failed: authentication tag verification failed")]
    AesGcmDecryptError,

    #[error("File I/O error {}during {operation}: {message}", path.as_ref().map(|s| format!("for '{s}' ")).unwrap_or_default())]
    FileIoError {
        path: Option<String>,
        operation: String,
        message: String,
...
impl From<RecryptErr> for IronOxideErr {
    fn from(recrypt_err: RecryptErr) -> Self {
        match recrypt_err {
            RecryptErr::InputWrongSize(_, expected_size) => IronOxideErr::WrongSizeError {
                actual_size: None,
            ...
            other_recrypt_err => IronOxideErr::RecryptError {
                msg: other_recrypt_err.to_string(),
            },
        }
    }
}
...
pub fn validate_name(name: &str, name_type: &str) -> Result<String> {
    ...
    if trimmed_name.trim().is_empty() || trimmed_name.len() > NAME_AND_ID_MAX_LEN {
        Err(IronOxideErr::ValidationError {
            field_name: name_type.to_string(),
            err: format!("'{trimmed_name}' must have length between 1 and 100"),
        })
    } else {
...
impl DeviceContext {
    ...
    pub(crate) fn account_id_internal(&self) -> &UserId {
        &self.auth.account_id
    }
    pub(crate) fn segment_id_internal(&self) -> usize {
        self.auth.segment_id
    }
    pub(crate) fn signing_private_key_internal(&self) -> &DeviceSigningKeyPair {
        &self.auth.signing_private_key
    }
    pub(crate) fn device_private_key_internal(&self) -> &PrivateKey {
...
impl DeviceContext {
    ...
    pub fn new(
        ...
        Self::new_from_parts(account_id, segment_id, device_private_key, signing_private_key)
    }
    /// ID of the device's owner
    pub fn account_id(&self) -> &UserId {
        self.account_id_internal()
    }
    /// ID of the segment
    pub fn segment_id(&self) -> usize {
        self.segment_id_internal()
    }
    /// Private signing key of the device
    pub fn signing_private_key(&self) -> &DeviceSigningKeyPair {
        self.signing_private_key_internal()
    }
    /// Private encryption key of the device
    pub fn device_private_key(&self) -> &PrivateKey {
        self.device_private_key_internal()
    }
}
...
impl DeviceContext {
    ...
    pub fn new(
        ...
        Self::new_from_parts(account_id, segment_id as usize, device_private_key, signing_private_key)
    }
    /// ID of the device's owner
    pub fn account_id(&self) -> UserId {
        self.account_id_internal().clone()
    }
    /// ID of the segment
    pub fn segment_id(&self) -> u64 {
        self.segment_id_internal() as u64
    }
    /// Private signing key of the device
    pub fn signing_private_key(&self) -> DeviceSigningKeyPair {
        self.signing_private_key_internal().clone()
    }
    /// Private encryption key of the device
    pub fn device_private_key(&self) -> PrivateKey {
        self.device_private_key_internal().clone()
    }
}
    ...
    pub(crate) fn deserialize_signed_public_key_cache(
        ...
        if signed_cache_bytes.len() < DEVICE_SIGNATURE_LENGTH {
            return Err(IronOxideErr::WrongSizeError {
                actual_size: Some(signed_cache_bytes.len()),
                expected_size: Some(DEVICE_SIGNATURE_LENGTH),
            });
        }
        ...
        if device.signing_private_key().verify(&cache, signature)? {
            ...
            Err(IronOxideErr::ValidationError {
                field_name: "signed public key cache".to_string(),
                err: "The signed public key cache failed signature verification.".to_string(),
            })
        }
    }
    ...
    fn try_from(key_bytes: &[u8]) -> Result<PublicKey> {
        if key_bytes.len() == RecryptPublicKey::ENCODED_SIZE_BYTES {
            ...
            Err(IronOxideErr::WrongSizeError {
                actual_size: Some(RecryptPublicKey::ENCODED_SIZE_BYTES),
                expected_size: Some(key_bytes.len()),
            })
        }
    }
}
...
impl PrivateKey {
    ...
    fn augment_user(&self, augmenting_key: &AugmentationFactor) -> Result<PrivateKey> {
        self.augment(augmenting_key, |msg| {
            IronOxideErr::UserPrivateKeyRotationError { msg }
        })
    }
    /// A convenience function to pass a user rotation error to `augment()`
    fn augment_group(&self, augmenting_key: &AugmentationFactor) -> Result<PrivateKey> {
        self.augment(augmenting_key, |msg| {
            IronOxideErr::GroupPrivateKeyRotationError { msg }
        })
    }
}
...
impl TryFrom<&[u8]> for DeviceSigningKeyPair {
    ...
    fn try_from(signing_key_bytes: &[u8]) -> Result<DeviceSigningKeyPair> {
        RecryptSigningKeypair::from_byte_slice(signing_key_bytes)
            ...
            .map_err(|e| {
                IronOxideErr::ValidationError { field_name: "DeviceSigningKeyPair".to_string(), err: e.to_string() }
            })
    }
}
...
impl TryFrom<&str> for Password {
    ...
    fn try_from(maybe_password: &str) -> Result<Self> {
        if !maybe_password.trim().is_empty() {
            ...
            Err(IronOxideErr::ValidationError {
                field_name: "maybe_password".to_string(),
...
pub(crate) mod tests {
    ...
    mod signed {
        ...
        fn signed_cache_too_short_fails() {
            ...
            assert!(matches!(result, Err(IronOxideErr::WrongSizeError { .. })));
        }
        #[test]
    ...
    async fn cache_lookup_fetch_error_propagates() {
        ...
        let result = get_keys_with_cache(&[uid], &cache, |_| async {
            Err(IronOxideErr::InitializeError {
                cause: "simulated fetch failure".into(),
            })
        })
        .await;
