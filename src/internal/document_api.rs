...
pub async fn decrypt_document2<CR: rand::CryptoRng + rand::RngCore>(
    auth: &RequestAuth,
    encrypted_doc: &[u8],
) -> Result<(AesEncryptedValue, DocumentMetadataResult), IronOxideErr> {
    let (doc_header, enc_doc) = parse_document_parts(encrypted_doc)?;
    document_get_metadata(auth, &doc_header.document_id)
        .await
        .map(|f| (enc_doc, f))
}
